...
%type <expr> expr expr_no_or expr_no_and expr_no_cmp expr_no_arith expr_no_geo expr_no_unary expr_no_exp expr_primary opt_expr expr_list expr_list_not_empty expr_group expr_no_assign expr_block expr_list_not_empty_b
...
decl			: ident TOKEN_COLON type TOKEN_ASSIGN expr TOKEN_SEMICOLON {$$ = decl_create($1,$3,$5,0,0);}
			...
			| ident TOKEN_COLON type TOKEN_ASSIGN stmt_block {$$ = decl_create($1,$3,0,$5,0);}
...
stmt_unmatched		: TOKEN_IF TOKEN_L_PAREN expr TOKEN_R_PAREN stmt {$$ = stmt_create(STMT_IF_ELSE,0,0,$3,0,$5,0);}
			...
			;
expr			: expr TOKEN_ASSIGN expr_no_assign {$$ = expr_create(EXPR_ASSIGN,$1,$3);}
			| expr_no_assign {$$ = $1;}
...
expr_group		: ident TOKEN_L_PAREN expr_list TOKEN_R_PAREN {$$ = expr_create(EXPR_FUNC,expr_create_name($1),$3);}
			| ident TOKEN_L_BRACKET expr TOKEN_R_BRACKET  {$$ = expr_create(EXPR_ARRAY,expr_create_name($1),$3);}
			| expr_primary {$$ = $1;}
			; 
expr_primary		: ident {$$ = expr_create_name($1);}
			| TOKEN_INTEGER_LITERAL {$$ = expr_create_integer_literal(atoi(yytext));}
			| TOKEN_STRING_LITERAL {char *buf; buf = strcpy(malloc(strnlen(yytext,256)+1),yytext); $$ = expr_create_string_literal(buf);}
